...
def get_active_loans_by_patron_pid(patron_pid):
    """Returns all the ongoing loans for a given patron."""
    search = get_loans_by_patron_pid(patron_pid) \
        .filter("terms", state=current_app.config["CIRCULATION_STATES_LOAN_ACTIVE"])
    return search
